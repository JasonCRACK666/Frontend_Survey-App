...
export class SurveyItemComponent {
  ...
  onDeleteSurvey(): void {
    this.surveyService.deleteSurveyById(this.surveyItem.id).subscribe(({ message }) => {
  ...
  onCopyLinkSurvey(): void {
    navigator.clipboard.writeText(`http://localhost:4200/survey/${this.surveyItem.id}/answer`)
