...
export class AnswerSurveyComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.surveyService.getSurvey(surveyId).subscribe(({ survey, questions }) => {
      this.survey = survey
      this.questions = questions
      questions.forEach(question => {
        this.answers.push(this.form.group({
  ...
  onSendAnswersSurvey(): void {
    ...
    console.log(this.responseSurvey.value.answers)
    this.answerService.answerSurvey(this.survey.id, this.responseSurvey.value.answers as AnswerEntity[]).subscribe(() => {
      this.loadSendingAnswers = false
